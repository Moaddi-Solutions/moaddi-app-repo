...
import { routes } from "~/app/CheckoutMoyasar/success";
...
const BoxesContainer = ({
  ...
  const { info, machine, machines, setMachines, setMachine, clearAll } =
    useMachine();
  ...
  const sorted = items?.sort((a, b) => a.boxNumber - b.boxNumber);
  const sortedItems =
    mode == "single" ? sorted?.slice(opened, opened + 1) : sorted;
  ...
  return done ? (
    ...
    <ScrollView
      ...
      className="my-6 px-6"
    >
      <Text className="mb-8 text-xl">{machine.name}</Text>
          ...
          box && (
            <View key={box._id}>
              <Card className="flex items-center justify-center p-4 m-4">
                <Text>
                  {[0, 1].includes(machine.type) ? box.name.slice(1) : box.name}
                </Text>
                {box.product && (
                  <>
                    <Image
