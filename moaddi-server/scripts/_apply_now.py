"""Apply critical fixes by reading exact current file contents."""
...
# Extract exact grant+stamp via regex so emdash/encoding can't drift
grant_m = re.search(
    r"const BASIC_ROLES = \[\"vendor\", \"customer\"\];\nconst FORBIDDEN_TENANT_GRANTS[\s\S]*?const canGrantRole = async \(req, role\) => \{[\s\S]*?\n\};\n",
    uc,
)
if not grant_m:
    raise SystemExit("canGrantRole block missing")
...
uc = uc[: grant_m.start()] + new_grant + uc[grant_m.end() :]
...
if not stamp_m:
    raise SystemExit("stampTenantOnCreate missing")
...
uc = uc[: stamp_m.start()] + new_stamp + uc[stamp_m.end() :]
...
sync_m = re.search(
    r"let syncSupplierMachines = async \(supplierId, machineIds\) => \{[\s\S]*?\n\};\n",
    m,
)
if not sync_m:
    raise SystemExit("syncSupplierMachines missing")
...
m = m[: sync_m.start()] + new_sync + m[sync_m.end() :]

create_old = '''  machine._id = "machine_" + machine.mac;
  machine.created = moment().utc().add(config.timeDifference, "hours");
  machine.updated = moment().utc().add(config.timeDifference, "hours");
'''

create_new = '''  if ("supportUserId" in machine || machine.supportUserId != null) {
...
if "normalizeSupportUserId(\n      machine.supportUserId" not in m:
    m = once(m, create_old, create_new, "machine create normalize")
...
'''
if "normalizeSupportUserId(\n      shop.supportUserId,\n      ownerId" not in s:
    s = once(s, shop_old, shop_new, "shop create normalize")
...
pk_m = re.search(
    r"export const permissionKey = \(row: Pick<RuleRow, 'action' \| 'subject'>\): string =>\n"
    r"  `\$\{row\.action\}:\$\{row\.subject\}`;\n",
    a,
)
if not pk_m:
    raise SystemExit("permissionKey missing")

new_pk = '''export const permissionKey = (
  row: Pick<RuleRow, 'action' | 'subject' | 'scope'>,
): string => `${row.action}:${row.subject}:${row.scope}`;
'''
a = a[: pk_m.start()] + new_pk + a[pk_m.end() :]
...
if "validateTenantRuleRows" not in a:
    ...
    if not vm:
        raise SystemExit("validateRuleRows missing")
    clamp = '''
...
if mx2 == mx:
    raise SystemExit("ruleMatrix keys not found")
# Ensure scope var exists in that loop — read context
# Around line 100 there should already be a `scope` from page.fixedScope || defaultScope
...
if "useSupportUserId({ shopId })" not in st:
    st = once(
        st,
...
if "effectiveCommissionPercent(row.machineId)" not in rev:
    rev = once(
        rev,
  ...
  return { data, total: data.length };''',
    ...
    if not re.search(r"(?:const|let) getRevenueByMachine\s*=\s*async", rev):
        rev = re.sub(
            r"((?:const|let) getRevenueByMachine\s*=\s*)(?!async)",
            r"\1async ",
            rev,
            count=1,
        )
